...
class HLTRModel:
    ...
    fnWordCharList = ""
    corpusFile = ""
    batchDir = "static/check_image/"

    def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, testing=False):
    ...
    def setupCTC(self):
        ...
        with tf.compat.v1.name_scope('CTC_Decoder'):
            ...
            if self.decoderType == DecoderType.BestPath:
                ...
                corpus = codecs.open(self.corpusFile, 'r').read()
    ...
    def get_batch_images(self):
        ...
        for i in range(1, self.batchSize):
            imgs.append(self.preprocess(cv2.imread(
                f"{self.batchDir}a ({i}).png", cv2.IMREAD_GRAYSCALE
            ), self.imgSize, enhance=False))
            return imgs

    def preprocess(self, img, imgSize, enhance=False, dataAugmentation=False):
    ...
    def recognize(self, image_path):
        img = self.preprocess(cv2.imread(
        ...
        imgs = self.get_batch_images()
        imgs = [img] + imgs
        batch = Batch(None, imgs)
        batch_recognized = self.inferBatch(batch)  # recognize text
        recognized = batch_recognized[0]
        corrected = correct_sentence(recognized)
